 ...
 */
#define PLAYER_NAME DJCUBITOv12
...
struct PLAYER_NAME : public Player {
  ...
  virtual void play () {
    ...
    for(int id : vf) {
      Pos pos = unit(id).pos;
      if(unit(id).health < 100 && not perill(pos.i,pos.j) && descans_f <= 2) {
	m[pos.i][pos.j] = true;
	descans_f++;
	command(id,None);
      }
    ...
    for(int id : vk) {
      Pos pos = unit(id).pos;
      if(unit(id).health < 200 && not perill(pos.i,pos.j) && descans_w == 0) {
	m[pos.i][pos.j] = true;
	descans_w++;
	command(id,None);
      }
