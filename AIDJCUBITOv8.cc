...
struct PLAYER_NAME : public Player {
  ...
  Pos moviment_w(int x, int y, Mvis m, vector<int>& equipo) {
   queue<Pos> p;
   bool trobat = false;
   ...
   while(not p.empty() && not trobat) {
     ...
     int i = pos.i;
     int j = pos.j;
     if(cell(i,j).id != -1 && unit(cell(i,j).id).player != 0 && equipo[unit(cell(i,j).id).player] == 0 && unit(cell(i,j).id).type != Witch && no_mata_unitat(i,j,m)) {
      trobat = true; 
      equipo[unit(cell(i,j).id).player]++;
     }
     for(int k = 0; k < 4; k++) {
  ...
  virtual void play () {
    ...
    vector<int> equipo(4,0);
    for(int id : vw) {
      Pos pos = unit(id).pos;
      Pos new_pos = moviment_w(pos.i, pos.j, m, equipo);
      if(new_pos.i == -1 && new_pos.j == -1) command(id,None);
      else {
