 ...
 */
#define PLAYER_NAME DJCUBITOv13
...
struct PLAYER_NAME : public Player {
  ...
  int distancia_k(int xo, int yo, int xn, int yn) {
   ...
   bool perill(int i, int j) {
     ...
     vector<int> js = {0,0,1,-1,1,-1,1,-1};
     for(int k = 0; k < 8; k++) {
       if(i+is[k] >= 0 && i+is[k] < 37 && j+js[k] >= 0 && j+js[k] < 37 && posicio_valida_k(i+is[k],j+js[k]) && cell(i+is[k],j+js[k]).id != -1 && unit(cell(i+is[k],j+js[k]).id).player != 0 && unit(cell(i+is[k],j+js[k]).id).type == Knight) 
	 return true;
     }
     return false;
   }
  ...
  virtual void play () {
    ...
    for(int id : vf) {
      Pos pos = unit(id).pos;
      if(unit(id).health < 100 && not perill(pos.i,pos.j) && descans_f <= 3) {
	m[pos.i][pos.j] = true;
	descans_f++;
	command(id,None);
      }
    ...
    for(int id : vk) {
      Pos pos = unit(id).pos;
      if(unit(id).health < 200 && not perill(pos.i,pos.j) && descans_k == 0) {
	m[pos.i][pos.j] = true;
	descans_k++;
	command(id,None);
      }
