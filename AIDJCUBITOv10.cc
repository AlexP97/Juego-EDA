 ...
 */
#define PLAYER_NAME DJCUBITOv10
...
struct PLAYER_NAME : public Player {
  ...
  Pos moviment_w(int x, int y, Mvis m, Mtrop& cab, int k) {
   queue<Pos> p;
   bool trobat = false;
   ...
   Pos pos;
   int player;
   if(total_score(1) >= total_score(2) && total_score(1) >= total_score(3)) {
     if(k == 0) 
       player = 1;
     else if(k == 1) { 
   ...
   else if(total_score(2) > total_score(1) && total_score(2) >= total_score(3) && total_score(1) >= total_score(3)) {
     if(k == 0) 
       player = 2;
     else if(k == 1) { 
   ...
   else if(total_score(3) > total_score(2) && total_score(3) > total_score(1)) {
     if(k == 0) 
       player = 3;
     else if(k == 1) { 
   ...
   while(not p.empty() && not trobat) {
     ...
     int i = pos.i;
     int j = pos.j;
     if(cell(i,j).id != -1 && unit(cell(i,j).id).player != 0 && unit(cell(i,j).id).player == player && unit(cell(i,j).id).type != Witch && no_mata_unitat(i,j,m) && cab[i][j] == 0) {
      trobat = true; 
      cab[i][j]++;
     }
     for(int k = 0; k < 4; k++) {
  ...
  virtual void play () {
    ...
    for(int id : vw) {
      Pos pos = unit(id).pos;
      Pos new_pos = moviment_w(pos.i, pos.j, m, cab, k);
      if(new_pos.i == -1 && new_pos.j == -1) command(id,None);
      else {
