 ...
 */
#define PLAYER_NAME DJCUBITOv17
...
struct PLAYER_NAME : public Player {
  ...
  int moviment_f(int x, int y, Mdist& md) {
    queue<Pos> p;
    bool trobat = false;
  ...
  Dir direccio_f(Pos pos, Mvis& mv) {
      vector<int> d(4,999);
      vector<int> is = {1,-1,0,0};
      vector<int> js = {0,0,1,-1};
      for(int k = 0; k < 4; k++) {
	Mdist md(37, vector<int>(37,999));
	if(posicio_valida_f(pos.i+is[k],pos.j+js[k]) && not mv[pos.i+is[k]][pos.j+js[k]] && cell(pos.i+is[k],pos.j+js[k]).id == -1) {
	  d[k] = moviment_f(pos.i+is[k], pos.j+js[k], md);
	}
      }
      int min = d[0];
  ...
  Dir direccio_k(Pos pos, Mvis& mv) {
      ...
      for(int k = 0; k < 8; k++) {
	Mdist md(37, vector<int>(37,999));
	if(posicio_valida_k(pos.i+is[k],pos.j+js[k]) && not mv[pos.i+is[k]][pos.j+js[k]] && (cell(pos.i+is[k],pos.j+js[k]).id == -1 || (cell(pos.i+is[k],pos.j+js[k]).id != -1 && unit(cell(pos.i+is[k],pos.j+js[k]).id).player != 0))) {
  ...
  int moviment_w(int x, int y, Mdist& md, Mvis mv) {
    queue<Pos> p;
    bool trobat = false;
    vector<int> is = {1,-1,0,0};
    vector<int> js = {0,0,1,-1};
    ...
    while(not p.empty() && not trobat) {
      ...
      int i = pos.i;
      int j = pos.j;
      if(cell(i,j).id != -1 && unit(cell(i,j).id).player != 0 && unit(cell(i,j).id).type != Witch && no_mata_unitat(i,j,mv) && not mv[i][j]) { 
	trobat = true;
      }
      for(int k = 0; k < 4; k++) {
  ...
  Dir direccio_w(Pos pos, Mvis& mv) {
      vector<int> d(4,999);
      vector<int> is = {1,-1,0,0};
      vector<int> js = {0,0,1,-1};
      for(int k = 0; k < 4; k++) {
	...
	if(posicio_valida_w(pos.i+is[k],pos.j+js[k]) && no_mata_unitat(pos.i+is[k],pos.j+js[k],mv)) {
	  d[k] = moviment_w(pos.i+is[k], pos.j+js[k], md, mv);
	}
      }
      int min = d[0];
      for(unsigned int i = 1; i < d.size(); i++) {
