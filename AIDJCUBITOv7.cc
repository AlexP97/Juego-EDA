...
struct PLAYER_NAME : public Player {
  ...
  int distancia_k(int xo, int yo, int xn, int yn) {
   ...
   Pos moviment_k(int x, int y/*, Mvis& ocupat*/) {
    queue<Pos> p;
    Mvis m(37, vector<bool> (37,false));
    vector<int> is = {1,-1,0,0,1,-1,-1,1};
    vector<int> js = {0,0,1,-1,1,-1,1,-1};
    ...
    while(not p.empty() && not trobat) {
      ...
      int i = pos.i;
      int j = pos.j;
      if(cell(i,j).id != -1 && unit(cell(i,j).id).type != Witch && unit(cell(i,j).id).player != 0/* && not ocupat[i][j]*/) {
	trobat = true;
	/*ocupat[i][j] = true;*/
      }
      for(int k = 0; k < 8; k++) {
  ...
  virtual void play () {
    ...
    for(int id : vk) {
      Pos pos = unit(id).pos;
      Pos new_pos = moviment_k(pos.i, pos.j/*,ocupat*/); 
      if(new_pos.i == -1 && new_pos.j == -1) command(id,None);
      else {
