 ...
 */
#define PLAYER_NAME DJCUBITOv14
...
struct PLAYER_NAME : public Player {
  ...
  Pos moviment_w(int x, int y, Mvis m, Mtrop& cab) {
   ...
   while(not p.empty() && not trobat) {
     ...
     int i = pos.i;
     int j = pos.j;
     if(cell(i,j).id != -1 && unit(cell(i,j).id).player != 0 && unit(cell(i,j).id).type != Witch && no_mata_unitat(i,j,m) && cab[i][j] == 0) {
      trobat = true; 
      cab[i][j]++;
     }
     for(int k = 0; k < 4; k++) {
  ...
  int distancia_k(int xo, int yo, int xn, int yn) {
   ...
   bool perill(int i, int j) {
     vector<int> is = {1,-1,0,0,1,-1,-1,1};
     vector<int> js = {0,0,1,-1,1,-1,1,-1};
     for(int k = 0; k < 8; k++) {
       if(i+is[k] >= 0 && i+is[k] < 37 && j+js[k] >= 0 && j+js[k] < 37 && cell(i+is[k],j+js[k]).type != Wall) {
	 if(cell(i+is[k],j+js[k]).haunted)
	   return true;
	 if(cell(i+is[k],j+js[k]).id != -1 && unit(cell(i+is[k],j+js[k]).id).player != 0 && unit(cell(i+is[k],j+js[k]).id).type == Knight)
	  return true;
       }
     }
     return false;
  ...
  virtual void play () {
    ...
    for(int id : vf) {
      Pos pos = unit(id).pos;
      if(unit(id).health < 100 && not perill(pos.i,pos.j) && descans_f <= 3 && not m[pos.i][pos.j]) {
	m[pos.i][pos.j] = true;
	descans_f++;
	command(id,None);
      }
    ...
    for(int id : vk) {
      Pos pos = unit(id).pos;
      if(unit(id).health < 100 && not perill(pos.i,pos.j) && descans_k == 0 && not m[pos.i][pos.j]) {
	m[pos.i][pos.j] = true;
	descans_k++;
	command(id,None);
      }
